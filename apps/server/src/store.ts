...
export function createStore(databasePath = config.databasePath): ChatStore {
  ...
  const findConversationStatement = database.prepare("SELECT id FROM conversations WHERE user_id = ? AND character_id = ?");
  const updateConversationStatement = database.prepare("UPDATE conversations SET updated_at = ? WHERE id = ?");
  const insertMessageStatement = database.prepare(`INSERT INTO messages (id, conversation_id, role, content, created_at) VALUES (?, ?, ?, ?, ?)`);
  const getMessagesStatement = database.prepare(`SELECT messages.id, messages.role, messages.content, messages.created_at FROM messages INNER JOIN conversations ON conversations.id = messages.conversation_id WHERE conversations.user_id = ? AND conversations.character_id = ? ORDER BY messages.sequence ASC`);
  const deleteConversationStatement = database.prepare("DELETE FROM conversations WHERE user_id = ? AND character_id = ?");
  ...
  return {
    getConversation(userId, characterId) {
      ...
      return rows.map(({ id, role, content, created_at: createdAt }) => ({ id, role, content, createdAt }));
    },
    saveMessage(userId, characterId, message) {
      const conversationId = ensureConversation(userId, characterId);
      insertMessageStatement.run(message.id, conversationId, message.role, message.content, message.createdAt);
      updateConversationStatement.run(message.createdAt, conversationId);
    },
    clearConversation(userId, characterId) { deleteConversationStatement.run(userId, characterId); },
