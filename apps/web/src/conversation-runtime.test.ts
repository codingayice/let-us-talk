...
function snapshot(messages: ChatMessage[] = []): ConversationSnapshot {
  return {
    ...
    summary: { ...summary, lastMessagePreview: messages.at(-1)?.content ?? "", lastMessageAt: messages.at(-1)?.createdAt ?? "2026-01-01T00:00:00.000Z" },
    messages,
    tasks: [],
  };
}
...
test("reconnect reloads the active conversation snapshot through the runtime seam", async () => {
  ...
  current.summary = { ...summary, lastMessagePreview: "Momo：重连后恢复", lastMessageAt: "2026-01-01T00:01:00.000Z" };
  realtime.reconnect();
  ...
  assert.equal(runtime.store.getSnapshot().summaries[0].lastMessagePreview, "Momo：重连后恢复");
  assert.deepEqual(realtime.joined, ["conversation-momo", "conversation-momo"]);
  runtime.stop();
});

test("stopping before initial loading prevents stale async state writes", async () => {
  let resolveList!: (value: ConversationSummary[]) => void;
  const api: ConversationApi = {
    list: () => new Promise((resolve) => { resolveList = resolve; }),
    async snapshotById() { return snapshot(); },
    async snapshotByCharacter() { return snapshot(); },
    async markRead() {},
    async hide() {},
    async restore() {},
    async clear() {},
  };
  const runtime = createConversationRuntime({ api, realtime: new FakeRealtime() });
  const started = runtime.start();
  runtime.stop();
  resolveList([summary]);
  await started;
  assert.equal(runtime.store.getSnapshot().activeConversationId, null);
  assert.deepEqual(runtime.store.getSnapshot().summaries, []);
});
